...
Enter and `H` use the available Inspect plugin, chosen in host order: Herdr, then tmux, then Ghostty. The bundled Herdr plugin uses Herdr 0.7.5+. The bundled tmux plugin activates whenever `TMUX` is set and writes a binding, so status, close, and focus all work; a tmux pane running inside a Ghostty window selects the tmux plugin. On macOS with Ghostty 1.3+ (TERM_PROGRAM=ghostty), the bundled Ghostty plugin splits the focused terminal using Ghostty's preview AppleScript API; status and close are unavailable because no binding is written. In a child-specific inspector, type ordinary guidance and press Enter to send it through the acknowledged steer channel; `steer <message>`, `status`, and `stop` remain available as explicit controls.
